...
def verify(root: Path, commit: str, tree: str):
    ...
    commands = [([str(Path(sys.executable).parent / (name + (".exe" if os.name == "nt" else ""))), "--help"], expected)
                for name, expected in (("gx3-fx5-inspect", 0), ("gx3-r-inspect", 0), ("gxw-inspect", 1),
                                       ("gxw-write", 0), ("gxw-reference-ir", 0), ("gxw-encode", 0))]
    commands += [([sys.executable, "-B", "-m", "gx_re_lab." + name, "--help"], 0) for name in LAB[:13]]
    commands += [([sys.executable, "-B", "-m", "gxw_pou_devmap"], 1)]
    commands += [([sys.executable, "-B", "-m", "unittest", "discover", "-s", str(root / path), "-v"], 0)
                 for path in ("packages/gx3-fx5/tests", "packages/gx3-r/tests", "packages/gxw-q/tests", "lab/tests", "tools/tests")]
    records = []
    ...
    return {"status": "PASS", "versions": VERSIONS, "origins": origins, "checks": records,
...
def main():
    ...
    result = verify(Path(__file__).resolve().parents[1], args.commit, args.tree)
